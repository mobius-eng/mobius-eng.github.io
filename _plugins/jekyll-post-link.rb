...
module Jekyll
  module Tags
    class PostLinkComparer
      ...
      private
      def post_slug(other)
        path = other.path.split("/")[0...-1].join("/")
        if path.nil? || path == ""
          other.slug
        else
          path + '/' + other.slug
    ...
    class PostLink < Liquid::Tag
      def initialize(tag_name, post, tokens)
        super
        args = post.gsub(/\s+/m, ' ').strip.split(" ")
        @orig_post = args[0].strip
        @anchor = args[1].strip
        begin
          @post = PostLinkComparer.new(@orig_post)
        rescue
          raise ArgumentError.new <<-eos
Could not parse name of post "#{@orig_post}" in tag 'post_link' (#{ @anchor }).
...
eos
      ...
      def render(context)
        ...
        site.posts.docs.each do |p|
          if @orig_post == p.basename_without_ext
            return "#{ p.url }\##{@anchor}"
          end
        end
